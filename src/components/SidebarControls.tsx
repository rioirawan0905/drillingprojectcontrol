...
import { Plus, Trash2, Sliders, Layers, DollarSign } from 'lucide-react';
        ...
        {activeTab === 'wbs' && (
          <div className="space-y-4 animate-fade-in">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={addWBSItem}
                className="text-xs font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1 bg-blue-50 hover:bg-blue-100/70 px-2 py-1 rounded-sm transition-colors cursor-pointer"
              >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      type="number"
                      value={item.budget}
                      onChange={(e) => updateWBSItem(item.id, 'budget', e.target.value)}
                      className="w-full text-xs bg-white text-slate-800 border border-slate-200 focus:outline-hidden focus:ring-1 focus:ring-blue-500 rounded px-2 py-1 font-mono"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={item.spent}
                      onChange={(e) => updateWBSItem(item.id, 'spent', e.target.value)}
                      className="w-full text-xs bg-white text-slate-800 border border-slate-200 focus:outline-hidden focus:ring-1 focus:ring-blue-500 rounded px-2 py-1 font-mono"
                    />
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateMonthlyCell(m.month, 'targetCashFlow', e.target.value)}
                          className="w-full text-xs bg-white text-slate-800 border border-slate-200 rounded px-1.5 py-0.5 font-mono"
                        />
                      </div>
                      <div>
